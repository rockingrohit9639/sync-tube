...
import { User, accounts, sessions, users, verificationTokens } from '../schema/auth'

export function pgDrizzleAdapter(client: InstanceType<typeof PgDatabase>): Adapter {
  return {
    async createUser(data: Omit<AdapterUser, 'id'> & { role: User['role'] }) {
      return await client
        .insert(users)
...
export function DrizzleAdapter(db: InstanceType<typeof PgDatabase>): Adapter {
